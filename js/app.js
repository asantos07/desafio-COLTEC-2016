var app = angular.module('eduCOLTEC', ['ngRoute','firebase']);

const hostAddress = "https://educoltec.firebaseio.com";

var ref = new Firebase(hostAddress);
 ...
 */
app.controller('VideosController', ['$sce', '$scope', '$location', 'Service', function($sce, $scope, $firebaseObject,  $location, service) {
  var self = this;
  self.videos = [];
  self.cursos = [];

  // recupera os vídeos
  service.get(hostAddress + '/videos.json', function(answer) {
    self.videos = answer;
    updateVideoCourse(0);
  });
  ...
  // recupera os cursos
  service.get(hostAddress + '/cursos.json', function(answer) {
    self.cursos = answer;
  });
